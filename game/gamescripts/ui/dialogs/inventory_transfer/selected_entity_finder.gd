...
func _find_station() -> RailNodeStation3D:
	...
	var closest_node_st_data: RailNodeStationData = self._find_closest_node_station(parent_station)
	return closest_node_st_data.station3d

func _find_closest_node_station(parent_station_obj: RailStationData) -> RailNodeStationData:
	var nearest_to_player_dist: float = 9999.9
	var nearest_to_player_node_st_obj: RailNodeStationData
	for node_station_obj: RailNodeStationData in parent_station_obj.node_stations:
