...
func put_on_connected_track(fork_node: RailNodeData):
	var set_to: int = fork_node.fork.set_to
	if fork_node.fork && set_to:
		Loggie.info("Switching to track with num %d" % set_to)
		self.current_track = RailTrackData.get_by_num(set_to)
