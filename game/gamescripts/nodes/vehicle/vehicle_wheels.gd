...
@export var current_track: RailTrack
@export var current_section: TrackSection

@export_storage var target: RailNode:
	get: return self.current_section.end

func _init(_vehicle: RailVehicle, _track: RailTrack, _start_index: int):
	self.vehicle = _vehicle
	self.current_track = _track
	self.current_node_i = _start_index
	# create section
	self.current_section = TrackSection.new()
...
func set_origin_point(_point_index: int):
	var start_node: RailNode = self.get_node_obj(_point_index)
...
func set_target_point(_point_index: int):
	var next_node: RailNode = self.get_node_obj(_point_index)
	self.current_section.set_end(next_node)
	if next_node:
		self.vehicle.rotate_to(next_node.position)

func get_node_obj(_i: int) -> RailNode:
