...
@export_storage var _containers: Array[OuterRailTrack] = []
...
func add_container(outer_track: OuterRailTrack):
...
func get_container_by_num(track_num: int) -> OuterRailTrack:
	var found: OuterRailTrack = self._containers_by_id.get(track_num, null)
	if ! found: Loggie.error("Cannot get OuterRailTrack %d; out of index?" % track_num)
	return found
#endregion
