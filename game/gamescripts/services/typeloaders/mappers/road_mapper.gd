...
static func add_points_from_json(_json_track: Dictionary, _road: RoadData):
	...
	for rail_node_dict: Dictionary in _json_track.points:
		...
		if rail_node_dict.has("roadCrossWith"):
			var connective_roads_arr = rail_node_dict.get("roadCrossWith", null)
			var cross: RoadCross = RoadCross.new(road_node, _arr_as_int_arr(connective_roads_arr))
			_road.crosses.append(cross)
			road_node.set_meta("cross", cross)
		_road.add_node(road_node)
		node_index += 1
		
static func _arr_as_int_arr(raw_arr: Array) -> PackedInt32Array:
	var int_arr: PackedInt32Array = PackedInt32Array()
	for raw_entry in raw_arr:
		int_arr.append(int(raw_entry))
	return int_arr
